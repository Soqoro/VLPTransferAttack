...
class ImageAttacker:
    def __init__(self, model, normalize, eps=8/255, steps=10, step_size=2/255):
        self.model = model
    ...
    def attack(self, images, txt_embeds, txt2img, device):
        ...
        for _ in range(self.steps):
            ...
            out = self.model.inference_image(normed)
            img_feat = out['image_feat']

            sim = img_feat @ txt_embeds.T
            label_mask = torch.zeros_like(sim)
            for i, idx in enumerate(txt2img):
                label_mask[idx, i] = 1

            loss = -(sim * label_mask).sum(dim=-1).mean()
...
class TextAttacker:
    ...
    def attack(self, net, texts, img_embeds):
...
class Attacker:
    ...
    def attack(self, imgs, texts, txt2img, device='cuda', **kwargs):
        # Step 1: Clean image embedding
        with torch.no_grad():
            img_output = self.model.inference_image(self.img_attacker.normalize(imgs))
            img_embed = img_output['image_feat'][txt2img]

        # Step 2: Text attack guided by clean image
        adv_texts = self.txt_attacker.attack(self.model, texts, img_embed)

        # Step 3: Get adversarial text embedding
        with torch.no_grad():
            txt_inputs = self.txt_attacker.tokenizer(adv_texts, padding='max_length', truncation=True, max_length=30, return_tensors='pt').to(device)
            txt_embed = self.model.inference_text(txt_inputs)['text_feat']

        # Step 4: Image attack guided by adversarial text
        adv_imgs = self.img_attacker.attack(imgs, txt_embed, txt2img, device)

        return adv_imgs, adv_texts, 0.0
